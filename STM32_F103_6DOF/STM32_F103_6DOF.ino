...
Arduino IDE Configuration:
  Board: "Generic STM32F103 series" → "Blue Pill F103C8"
...
6-AXIS SYSTEM WIRING (identical on both boards):
  ...
  Compile this sketch TWICE:
    1. For Master: set CONTROLLER_MODE = CONTROLLER_MODE_MASTER in dma_stepper_hal.h
    2. For Slave:  set CONTROLLER_MODE = CONTROLLER_MODE_SLAVE in dma_stepper_hal.h
=============================================================================
*/
// 2026-08-18: Merged 6-axis SPI Master/Slave with advanced limit switch config.
//             - Protocol extended to 6 axes (PCCMD uses AXES_TOTAL).
//             - Master handles axes 0-2 locally, forwards 3-5 to Slave via SPI.
//             - sendAllStates() reports all 6 axes (3 local + 3 from Slave).
//             - FIXED: Added {} around switch cases to prevent C++ initialization scope errors.
...
// =============================================================================
// PIN ASSIGNMENTS (identical on Master and Slave)
// =============================================================================
...
enum COMMAND : uint8_t {
  CMD_HOME = 0, CMD_MOVE = 1, CMD_SET_SPEED = 2, CMD_DISABLE = 3,
  CMD_ENABLE = 4, CMD_GET_STATE = 5, CMD_CLEAR_ALARM = 6, CMD_PARK = 7,
  SET_ALARM = 8, CMD_SET_LOW_SPEED = 9, CMD_SET_ACCEL = 10, CMD_MOVE_SH = 11,
  CMD_SET_PID_KP = 0x0C, CMD_SET_PID_KI = 0x0D, CMD_SET_PID_KD = 0x0E,
  CMD_SET_PID_KS = 0x0F, CMD_SET_PID_ENABLE = 0x10, CMD_SET_PID_BLEND = 0x11,
  CMD_GET_PID_STATE = 0x12, CMD_STORE_PID = 0x13, CMD_RESTORE_PID = 0x14
};
...
enum PID_FLAGS : uint8_t {
  PID_NONE = 0, PID_ENABLED = 1, PID_MASTER_SYNC = 1 << 1, PID_DIAG_ENABLED = 1 << 2
};
...
PID_STATE pidGlobal = {
  .version = 1, .flags = PID_FLAGS::PID_ENABLED, .blend = 100,
  .Kp = 15.0f, .Ki = 0.0f, .Kd = 0.02f, .Ks = 0.30f
};
...
void processCommand() {
  switch (pccmd.cmd) {
...
#if (CONTROLLER_MODE == CONTROLLER_MODE_MASTER)
      for (int i = 0; i < AXES_PER_BOARD; i++) {
        DMAStepper_SetMaxSpeed(i, (uint32_t)pccmd.data[0]);
      }
    ...
    case SET_ALARM: {
      ...
      for (int i = 0; i < AXES_PER_BOARD; i++) {
        AxisState* ax = DMAStepper_GetAxis(i);
        if (ax) { ax->mode = MODE_ALARM; ax->homed = false; }
      }
#endif
      break;
    }
    ...
    case CMD_SET_PID_ENABLE: {
      for (int i = 0; i < AXES_PER_BOARD; i++) {
        DMAStepper_SetPIDEnable(i, pccmd.data[0] != 0);
      }
